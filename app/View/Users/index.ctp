<script>$(document).ready(function($) {$('html').addClass('atriumblur');});</script>
	...
	<div class="box">
		<h2><?php echo $this->Html->image('icons/home.png',array('class'=>'icons-list-img')).' '.$staffMember[0]['User']['fullname']?></h2>
		<div><ul>
			<li>Area | <?php echo ClassRegistry::init('Option')->getOption($staffMember[0]['User']['option_id']);?></li>
		</ul></div>
	</div>
	...
	<div class="box">
		...
		<div><ul>
			<?php
				echo '<li class="icons-li">'.$this->Html->image('icons/star-outline.png',array('url'=>array('controller'=>'targets','action'=>'add',$staffMember[0]['User']['id']),'title'=>'manage targets','class'=>'icons-list-img')).$this->Html->link('Manage Targets',array('controller'=>'targets','action'=>'add',$staffMember[0]['User']['id'])).'</li>';
			?>
		</ul></div>
	</div>
	...
	<div class="box">
		...
		<div><ul>
			<?php
				echo '<li class="icons-li">'.$this->Html->image('icons/lock-closed.png',array('url'=>array('controller'=>'notes','action'=>'view',$staffMember[0]['User']['id']),'title'=>'teacher notes','class'=>'icons-list-img')).$this->Html->link('Teachers Notes',array('controller'=>'notes','action'=>'view',$staffMember[0]['User']['id'])).'</li>';
			?>
		</ul></div>
	</div>
